...
(function(){
    ...
    var step=0;
    oBoxInner.innerHTML+='<div><img src="hc2016_banner_bg_2.jpg" alt=""/></div>';
    oBoxInner.style.width=aDiv.length*aDiv[0].offsetWidth+'px';
    //1）图片自动轮播
    clearInterval(autoTimer);
    var autoTimer=setInterval(autoMove,1583);
    function autoMove(){
        if(step>=aDiv.length-1){
            step=0;
            utils.css(oBoxInner,'left',-step*1583);
        }
        step++;
        //utils.css(oBoxInner,'left',-step*1000)
        //小技巧：在使用运动库前，先写好效果，再加运动
        zhufengAnimate(oBoxInner,{left:-step*1583},1000,0);
        bannerTip();
    }
    //2）圆按钮也是自动轮播
    function bannerTip(){
    ...
    oBox.onmouseout=function(){
        autoTimer=setInterval(autoMove,1583);
    ...
    function handleChange(){
        for(var i=0; i<aLi.length; i++){
            ...
            aLi[i].onclick=function(){
                ...
                zhufengAnimate(oBoxInner,{left:-step*1583},1000);
                bannerTip();
            }
        }
    }
